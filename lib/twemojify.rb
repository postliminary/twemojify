module Twemojify
  ...
  def self.parse(str, options = {})
    ...
    str.gsub(REG_EX) do |match|
      # TODO Refactor
      image_name = match.codepoints.map { |a| a.to_s(16) }.join('-')
      "<img class=\"emoji\" draggable=\"false\" alt=\"#{match}\" src=\"#{options[:base]}#{options[:size]}/#{image_name}#{options[:ext]}\">"
    end
  end
